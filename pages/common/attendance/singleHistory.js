...
export default (props) => {
  ...
  const courseAndParticipantsGQLQuery = useQuery(FETCH_PARTICIPANTS_QUERY, {
    onError(err) {
  ...
  useEffect(() => {
    if (courseAndParticipantsGQLQuery.data) {
      setParticipants(courseAndParticipantsGQLQuery.data.getParticipants);
    }
  ...
  useEffect(() => {
    if (trxListInAttendanceGQLQuery.data) {
      const currAbsentees = participants.filter((participant) => {
        ...
        const result =
          trxListInAttendanceGQLQuery.data.getTrxListInAttendance.filter(
