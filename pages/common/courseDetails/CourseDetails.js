...
import { KICK_PARTICIPANT_MUTATION } from "../../../graphql/mutation";
import {
  FETCH_COURSE_QUERY,
...
export default (props) => {
  const columns = [
    ...
    {
      title: <strong>Roll Number</strong>,
      dataIndex: "cardID",
      key: "cardID",
      align: "center",
  ...
  const fetchCourseGQLQuery = useQuery(FETCH_COURSE_QUERY, {
    onError(err) {
      CheckError(err);
    },
    variables: {
  ...
  const fetchParticipantGQLQuery = useQuery(FETCH_PARTICIPANTS_QUERY, {
    onError(err) {
      CheckError(err);
    },
    variables: {
        ...
        <Content>
          <Card className="courseDetails__card">
            {fetchCourseGQLQuery.data && (
              <CourseDetailCard course={fetchCourseGQLQuery.data.getCourse} />
            )}
          </Card>
          <Divider
            orientation="left"
